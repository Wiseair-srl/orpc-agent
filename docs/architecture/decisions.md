...
> **Status:** All ADRs below are **Accepted** and describe the 1.0 implementation as built (several carry as-built addenda). Revisiting one requires a superseding ADR.
...
1. **Startup footgun warnings.** *(Superseded by [ADR-016](#adr-016-runtime-policies-are-part-of-the-governance-contract) §9: the `warnings` flag is removed — a warning is answered by making the implicit choice explicit.)* `createAgentRuntime` accepted `warnings?: boolean` (default `true`) and emits `console.warn` at construction for two statically detectable production footguns: (a) capabilities with `meta.approval.required` while the coordinator is the in-memory default *and* no inline handler is configured (pure inline confirmation is an explicit choice that legitimately never persists); (b) write/destructive/external capabilities exposed to `aiSdk`/`mcp` with zero audit sinks. Never fatal; static knowledge only (policy-driven gates are opaque at construction).
...
9. **A startup warning for the same blind spot in core, and `warnings` is removed.** `emitStartupWarnings` keyed on `meta.approval.required`, so an application gating only via policy got no warning that its approvals were suspending into the restart-amnesiac default coordinator. A new condition covers it: policies configured, write-capable capabilities on model surfaces, and no coordinator named.

    Sizing that condition was a trade-off only because `warnings?: boolean` existed. Firing on every write catches the case where an application declares a gated capability `write` rather than `destructive` — but it also fires for the ordinary rate-limit or tenant-isolation policy that never returns `requireApproval`, and those applications' only recourse was `warnings: false`, which would also disable the unrelated and valuable audit-sink warning. A noisy warning that takes working warnings down with it has negative value.

    The trade-off dissolves once the flag goes. **Every warning already fires only where a decision was left implicit, and every one is answered by making it** — naming `approvals.coordinator` (`createInMemoryApprovalCoordinator()` is a legitimate answer), or naming an audit sink (`audit: () => {}` states deliberately that nothing is recorded). `warnings: false` was therefore a second way to express what a configuration choice already expresses, and a worse one: global, outliving the reason it was added, and hiding the decision from whoever reads the code next. With it gone, a noisy warning can no longer cause collateral damage, so the condition is sized for coverage: all write-capable side effects. ADR-014's `warnings?: boolean` is superseded.
...
**Status:** accepted · v2.0 (planned and sequenced as 1.1)

**Context.** `describe` had no way to return a subset. A host that wants a route-scoped catalog had to fetch every capability the actor can see and discard most of it, and the only shaping available — `filter` on `toAISDKTools` — runs *after* `describe` returns. That trims tokens and nothing else: the registry walk, every discovery-phase policy evaluation, and a `structuredClone` of every input schema were already paid for. A host that re-composes per step, per actor — which the per-actor composition rule requires — paid it on every step of every turn.

The 1.0 scope was fixed by a ~85-capability finance app. The second production consumer is a dashboard host targeting ~300 capabilities across six tag groups, where a step needs one group. Three latent sharp edges in the same code path become load-bearing at that size: an unbounded id array in a per-discovery audit event, serial policy evaluation, and a policy timeout budget that is per capability rather than per discovery.

**Decision.**

1. **`describe(surface, { actor, context, scope })`, filtered before any policy runs.** The pipeline order is normative: exposure filter (SI-1) → **scope filter** → discovery-phase policies (SI-7) → schema conversion → audit. Placing the filter at step 2 rather than after step 3 is the entire point. At step 3 it saves tokens; at step 2 it saves work.

2. **Scope matches on `tags`, because tags already exist.** `CapabilityMeta.tags` is declared today and `CapabilityDescriptor.tags` is already returned, so nothing new enters the metadata model and no existing capability migrates. `tags` is **ANY** — a capability carrying any listed tag matches, and an intersection is expressible with `ids`. **An untagged capability matches no `tags` scope.** The alternative (untagged matches everything) would avoid silently hiding capabilities from a consumer that adopts scope before tagging, at the cost of making every scope a lie about what it returns; predictable and opt-in wins, documented loudly in [concepts/capabilities.md](../concepts/capabilities.md#tags-are-how-a-catalog-gets-asked-for-in-pieces) and [reference/runtime.md](../reference/runtime.md#scope-discovery-shaping-never-an-authority-boundary). An object carrying neither key does not narrow, so `scope: {}` reads as omitting it; a key present with an empty array is a constraint that matches nothing.
...
4. **`capabilities.discovered` carries constant-size data.** `{ count, surface, digest }`, where `digest` hashes the sorted id list. At 300 capabilities the id array was ~6 KB emitted per discovery — per step, per turn, per concurrent user — and consumers that forward audit events to a browser panel carried it over the wire every time. A digest still answers what the event exists to answer (*did this actor's visible catalog change, and when*) and correlates across events without carrying the contents. The **algorithm is deliberately not part of the contract**: the guarantee is equality (equal digests ⇒ equal catalogs), so digests may be compared but never parsed, inverted, or stored as identifiers. Committing to an algorithm would buy cross-version comparability that no consumer asked for and freeze an implementation detail of a routine event.
...
5. **Discovery-phase policies evaluate with bounded concurrency** — `defaults.policyConcurrency`, default 16. Fail-closed semantics are unchanged: each capability gets its own batch, its own shared deadline, and its own outcome, so a policy error still excludes exactly its own capability (SI-7) and cannot leak into a neighbour's. Descriptor order stays registry order regardless of completion order. Today's consumers write synchronous discovery policies where this changes nothing; the moment one does I/O, 300 sequential round trips per discovery is what the API's shape was quietly inviting.

6. **`defaults.discoveryBudgetMs` bounds the whole discovery, and expiry fails.** `policyTimeoutMs` bounds one capability's batch, so the worst case at 300 capabilities was 300 × `policyTimeoutMs` before `describe` returned. The budget (default 30 s, the same ceiling an execution gets) is checked before each batch and clamps each batch's own timeout to what is left. On expiry `describe` **throws rather than returning a partial catalog**: a silently short catalog is indistinguishable from "the actor lost access", which is precisely the confusion the framework's concealment rules exist to avoid.

    It throws `CapabilityError` with the existing `TIMEOUT` code at `stage: "discovery"` — no new error code. A distinct code would have to be added to a closed union every consumer switches on, to say something `TIMEOUT` + `stage` already says exactly. `describe` has no result envelope (it is not an invocation), so a rejected promise is the only shape available; programmer errors still reject with `TypeError`.

7. **`@orpc-agent/ai-sdk` forwards `scope` verbatim, and `filter` is unchanged.** Both exist because they answer different questions, and the doc says which: **`scope` decides what gets discovered; `filter` decides what survives discovery.** Neither is authorization. `@orpc-agent/testing`'s `describe` forwards it too, so the supported test seam can exercise the feature.

**Consequences.** Purely additive for `describe` and for the adapter: omitting `scope` returns the 1.0 result, in the same order. Breaking for any consumer reading `data.capabilityIds` off `capabilities.discovered` — in this repository, the Mastra example's audit panel, which now reads `data.count`; the CLI is not a consumer of this event (it reads registry values and never runs a discovery). Behaviourally new: `describe` can now reject with a `CapabilityError`, where before only a `TypeError` was possible.

**This ships as 2.0.0, not the 1.1.0 the plan named.** Decision 4 removes a documented field from a documented event type, and [the release process](../contributing/release-process.md) says a breaking change never ships in a minor whatever the migration notes say — that allowance ended at 1.0. The alternative the same document offers is a deprecation cycle: emit `capabilityIds` alongside `count`/`digest` through 1.x and drop it at 2.0. That was rejected, and the reason is specific rather than procedural. The deprecated field would be **the unbounded array whose cost is the entire justification for the change**, so a compliant 1.1 would ship the fix and none of the relief, while carrying two payload shapes for a full major cycle. Deleting the second shape is what [ADR-016](#adr-016-runtime-policies-are-part-of-the-governance-contract) did to `registry`/`policies` and to `warnings`, on the same reasoning. A version number is the cheap part; the migration is one field read, and `verbose: true` restores the old payload verbatim for anyone who cannot make it now.

Everything else in this ADR is additive and would have been a legitimate minor on its own. They travel together because the release train is `linked`.

Bounded concurrency and a global budget are defaults, not opt-ins, because both replace an unbounded behaviour with a bounded one — and a knob nobody sets is a knob that protects nobody. Applications that genuinely want serial evaluation set `policyConcurrency: 1`.

**Unresolved.** Whether `scope` should reach [`@orpc-agent/mcp`](../adapters/mcp.md), where a large catalog has the same discovery cost. It is deliberately out of 1.1: MCP's `tools/list` is driven by the protocol rather than by a per-step host decision, so the natural shape is server construction configuration rather than a per-call argument, and that is a different design question than the one this ADR answers.
